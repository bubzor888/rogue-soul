...
Key signals: `codex_entry_added`, `item_broken`.

#### Scenario: Domain-presentation decoupling
- **WHEN** an item breaks in domain code
- **THEN** the domain emits SignalBus.item_broken; the presentation layer reacts without the domain knowing a UI exists
