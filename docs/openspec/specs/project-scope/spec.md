...
The game SHALL be fully playable with a complete UI layer. All MVP1 systems remain functional; the player can now play through a Pilgrim run using the actual game interface on desktop.

**In scope:**
- All MVP1 systems
- Full presentation layer: combat screen, action bar, omen display, inventory, door selection
- Portrait-first layout (see `lld-platform-constraints`)
...
**Out of scope (deferred):** Audio, additional vessels, additional floors, meta-progression.

#### Scenario: MVP2 complete
- **WHEN** MVP2 is complete
- **THEN** a player can complete a full Pilgrim run using the game UI on desktop with no missing interactions
...
**Out of scope (deferred):** Meta-progression (post-run unlocks, Soul Codex), audio, mobile port.
