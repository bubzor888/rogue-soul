...
- **Departure:** After each drain, if `companion_timer <= 0`, The Shadow departs immediately. Departure may occur mid-combat.
...
- **Activation source:** Fires regardless of what caused the vessel to reach 0 HP — direct attack, status effect tick (Burning, Poisoned), companion drain (The Shadow targeting the vessel is not possible as companions cannot target the player, but future mechanics may apply), or any other HP-reducing effect.
