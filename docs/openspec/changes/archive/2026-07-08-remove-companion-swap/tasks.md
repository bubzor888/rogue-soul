...
- [x] 2.1 Re-read `HLD-MF-004` and `LLD-FLOOR-BEATS-003` after archive and confirm the "one companion encounter per floor" guarantee is now stated as a structural fact (Memory Fragment never draws Companion Encounter while an unfired Worn Map is held or after any companion has been offered), not as a player-facing fallback. Confirmed in both synced main specs.
...
- [x] 3.1 Run `/opsx:archive` once the above is verified, to fold these deltas into `docs/openspec/specs/hld-memory-fragments/spec.md` and `docs/openspec/specs/lld-floor/spec.md`.
